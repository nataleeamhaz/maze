...
struct maze_room *get_neighbor(int num_rows, int num_cols,
                               ...
                               struct maze_room *room, Direction dir) {
                                   if (dir == SOUTH && (is_in_range(room -> row + 1, room -> column, num_rows, num_cols)) == 1){
                                       return &maze[room -> row + 1][room -> column];
                                   }else if (dir == NORTH && (is_in_range(room -> row - 1, room -> column, num_rows, num_cols)) == 1){
                                       return &maze[room -> row - 1][room -> column];
                                   } else if (dir == EAST && (is_in_range(room -> row, room -> column + 1, num_rows, num_cols)) == 1 ){
                                       return &maze[room -> row][room -> column + 1];
                                   } else if (dir == WEST && (is_in_range(room -> row, room -> column - 1, num_rows, num_cols)) == 1){
                                       return &maze[room -> row][room -> column - 1];
                                   } else{
                                       return NULL;
                                   }
